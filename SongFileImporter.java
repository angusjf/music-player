...
public class SongFileImporter {
    ...
    public void metadataExtractionComplete() {
        ...
        //check for new album
        int fileAlbumId = getAlbumId((String)metaData.get("album"), fileArtistId);

        int fileTrackNumber = /*Integer.parseInt(*/(Integer)metaData.get("track number")/*)*/;
	...
	/*
        ...
        PreparedStatement statement = Main.database.createStatement("SELECT Id FROM Genres WHERE Name = ?");
        try {
            statement.setString(1, genreString);
        }  catch (SQLException ex) {
            System.out.println("- setting ? error");
        }
        ResultSet results = Main.database.runSelectStatement(statement);
	...
	*/
    ...
    private int getArtistId(String artistName) {
	    /*
        PreparedStatement statement = Main.database.createStatement("SELECT Id FROM Artists WHERE Name = ?");
        try {
